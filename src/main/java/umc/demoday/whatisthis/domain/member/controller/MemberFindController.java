...
public class MemberFindController {
    ...
    public ResponseEntity<CustomResponse<Void>> verifyCode(
        ...
        ResponseCookie cookie = ResponseCookie.from("resetToken", rt.value())
                .httpOnly(true)
                .secure(true)        // 로컬 http 테스트면 false, 배포는 true 권장
                .sameSite("None")     // 크로스 도메인이면 "None" + secure(true)
                .path("/")
                .maxAge(rt.ttl())
                .build();
    ...
    public CustomResponse<Void> resetPassword(
        ...
        // 삭제 쿠키 내려서 프론트/브라우저에서 정리
        ResponseCookie delete = ResponseCookie.from("resetToken", "")
